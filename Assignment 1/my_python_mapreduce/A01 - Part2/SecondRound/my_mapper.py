...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
	...
	for line in my_input_stream:
		...
		if ((int(my_tuple[0]) == 0) and (int(my_tuple[5]) == 0) and (my_tuple[1] == "Fitzgerald's Park")):
			...
			dateOnly = datetime.datetime.strptime(my_tuple[4], dateformat)			
			res.append(dateOnly.hour)
			days.append(calendar.day_name[(datetime.datetime.strptime(my_tuple[4], dateformat)).weekday()])
			

	count = Counter(res)
		
	for item in count:
		my_output_stream.write( days[0]+"_"+str(item)+"\t"+"("+str(count[item])+")" + "\n")
