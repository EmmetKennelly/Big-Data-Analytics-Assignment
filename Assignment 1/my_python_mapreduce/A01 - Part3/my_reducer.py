...
def my_reduce(my_input_stream, my_output_stream, my_reducer_input_parameters):
    ...
    for line in my_input_stream:
        ...
        if(date == "" and time == ""):
            date = my_tuple[0]
            time = my_tuple[1]
            recentTime = my_tuple[1]
        elif(date == my_tuple[0] and dt.datetime.strptime(my_tuple[1], '%H:%M:%S') - dt.datetime.strptime(time, '%H:%M:%S') <= timedelta(minutes=5)):
                numOF = numOF +1
                time = my_tuple[1]
        else: 
                output = (date + "\t(" + recentTime + ", " + str(numOF)+")\n")
