...
def my_model(ssc, monitoring_dir, window_duration, sliding_duration, time_step_interval):
    ...
    filter_Stream = my_tupleStream.filter(lambda line: line[0] == "0" and line[5] == "0")
    short_Stream = filter_Stream.map(lambda line: (line[1], 1))
    reduced_Stream = short_Stream.reduceByKey(lambda a, b: a + b)
    ordered_Stream = reduced_Stream.transform(lambda newValue: newValue.sortBy(lambda a: a[1], ascending=False))
